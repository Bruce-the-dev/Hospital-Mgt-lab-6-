...
public class PrescriptionController {
        ...
        @Operation(summary = "Create a new prescription", description = "Creates a prescription for an existing appointment", responses = {
        ...
        @Operation(summary = "Get prescription by ID", description = "Fetch a prescription by its unique ID", responses = {
        ...
        public ResponseEntity<Prescription> getPrescriptionById(
                        @Parameter(description = "Prescription ID", example = "1") @PathVariable @Min(1) Long id) {
                return ResponseEntity.ok(prescriptionService.getPrescriptionById(id));
        }

        @Operation(summary = "Update prescription", description = "Update issued date, notes, or medications of a prescription", responses = {
        ...
        @Operation(summary = "Delete prescription", description = "Delete a prescription by ID", responses = {
        ...
        @Operation(summary = "Get all prescription reports (paginated)", description = "Returns paginated prescription reports with sorting support")
        @GetMapping("/reports")
        ...
        @Operation(summary = "Get prescription reports by patient", description = "Fetch all prescription reports for a specific patient")
        @PreAuthorize("hasAnyRole('ADMIN','DOCTOR','NURSE','RECEPTIONIST')")
        @GetMapping("/reports/patient/{patientId}")
        public ResponseEntity<List<PrescriptionReportDTO>> getReportsByPatient(
                        @Parameter(description = "Patient ID", example = "5") @PathVariable @Min(1) Long patientId) {
                return ResponseEntity.ok(
                                prescriptionService.getReportsByPatient(patientId));
        }

        @Operation(summary = "Get prescription reports by doctor", description = "Fetch all prescription reports written by a specific doctor")
        @GetMapping("/reports/doctor/{doctorId}")
        @PreAuthorize("hasAnyRole('ADMIN','DOCTOR','NURSE','RECEPTIONIST')")
        public ResponseEntity<List<PrescriptionReportDTO>> getReportsByDoctor(
                        @Parameter(description = "Doctor ID", example = "3") @PathVariable @Min(1) Long doctorId) {
                return ResponseEntity.ok(
